...
public class OOPLinkedList<T> implements OOPList<T> {
    protected class OOPLinkedListElement {
        ...
        public OOPLinkedListElement(T value, OOPLinkedListElement next) {
            this.value = value;
            this.next = next;
        }
    ...
    public void insert(T elem) { // insert at front
        this.head = new OOPLinkedListElement(elem, head);
    }

    public void append(T elem) { // append to end of list
        if (head == null) {
